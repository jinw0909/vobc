...
export default function Profile() {
    ...
    useEffect(() => {
        ...
        async function loadProfile() {
            try {
                ...
                setUserProfile({
                    walletAddress:
                        normalizeOptionalString(data.walletAddress) || '',
                    nickname:
                        normalizeOptionalString(data.nickname),
                    email:
                        normalizeOptionalString(data.email),
                    bio:
                        normalizeOptionalString(data.bio),
                    profileImageUrl:
                        normalizeOptionalString(data.profileImageUrl),
                })
            } catch (error) {
                console.error(
                    '[ProfileContent loadProfile error]',
                    error
                )
    ...
    const handleProfileImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const res = await authFetch(
                `${API_BASE_URL}/web3/profile/image`,
                {
                    method: 'POST',
                    body: formData,
                }
    ...
    const saveProfile = async () => {
        ...
        try {
            const res = await authFetch(
                `${API_BASE_URL}/web3/profile/update`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type':
    ...
    const requestPortfolio = async (
        targetReAuthToken: string
    ) => {
        const res = await authFetch(
            `${API_BASE_URL}/web3/profile/portfolio`,
            {
                method: 'GET',
                headers: {
                    'X-Reauth-Token':
        ...
        if (!res.ok) {
            throw new Error(
                `portfolio 요청 실패: ${res.status} / ${text}`
            )
        }
    ...
    const runReAuthAndRequestPortfolio = async () => {
        ...
        try {
            ...
            const numericChainId = currentChainId.startsWith('0x')
                ? Number.parseInt(currentChainId, 16)
                : Number(currentChainId)

            if (Number.isNaN(numericChainId)) {
                throw new Error(`chainId 형식이 올바르지 않습니다: ${currentChainId}`)
            }
            ...
            // const numericChainId = getNumericChainId()

            const nonceRes = await authFetch(
                `${API_BASE_URL}/web3/auth/reauth/nonce`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type':
            ...
            const verifyRes = await authFetch(
                `${API_BASE_URL}/web3/auth/reauth/verify`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type':

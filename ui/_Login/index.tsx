...
export default function Login({onConnectSuccess, onLoginSuccess, onLogout, onDisconnect}: LoginProps) {
    ...
    const isConnected = !!account && !!connectionType && status === 'connected';
    const isLoggedIn = !!accessToken;

    const connectButtonLabel = isConnected ? '연결 해제' : '연결';
    const authButtonLabel = isLoggedIn ? '로그아웃' : '로그인';
    const authButtonDisabled = !isConnected;
    ...
    const handleDisconnectClick = async () => {
        ...
        await disconnectConnector('지갑 연결을 직접 해제했습니다.');
        await onDisconnect?.();
    }
    ...
    const requestVobBalance = async () => {
        try {
            setApiResult('/web3/vob-balance 요청 중...');
            ...
            setApiResult(`/web3/vob-balance 응답\nstatus: ${res.status}\n\n${body}`);
        } catch (error: any) {
            setApiResult(`/web3/vob-balance 요청 에러: ${error?.message || 'unknown error'}`);
        }
    };
    ...
    return (
        <main className={styles.main}>
            <div className={styles.card}>
                ...
                <h1 className={styles.title}>Web3 로그인 테스트</h1>
                ...
                {/*<button onClick={loginWithWeb3} className={styles.buttonSecondary}>*/}
                {/*    Web3 로그인 시도*/}
                {/*</button>*/}

                <button onClick={requestWeb3Test} className={styles.buttonSecondary}>
                    /web3/test 요청
                </button>

                <button onClick={requestVobBalance} className={styles.buttonSecondary}>
                    /web3/portfolio 요청
                </button>

                {/*<button onClick={logoutWeb3} className={styles.buttonSecondary}>*/}
                {/*    Web3 로그아웃*/}
                {/*</button>*/}

                <button onClick={refreshTokenTest} className={styles.buttonSecondary}>
                    Refresh 테스트
                </button>

                <div className={styles.resultBox}>
                    <strong>API Result</strong>
                    <pre className={styles.resultText}>{apiResult || '아직 요청 없음'}</pre>
                </div>
            </div>
        </main>
    );
